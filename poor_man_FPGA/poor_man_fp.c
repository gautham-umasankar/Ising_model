...
#define SYNC_BUFFER_SIZE 500

float ascale = 1.005;
int p_step = 1000;
int trig_delay; // + 500
int t1 = 6000;  //16384+3900;
...
void read_J()
{
    ...
    while (fgets(line, 30, j_file) != 0)
    {
        ...
        J_fp[row-1][column-1] = -BETA*value;
        J_fp[column - 1][row - 1] = -BETA*value;
        // printf("J_fp[%d][%d]=%f\n",row-1,column-1,J_fp[row-1][column-1].to_float());
        J[row - 1][column - 1] = -BETA*value;
        J[column - 1][row - 1] = -BETA*value;
        OG_J[row - 1][column - 1] = value;
        OG_J[column - 1][row - 1] = value;
        if(value > max_weight)
            max_weight = value;
        if(value < min_weight)
            min_weight = value;
    ...
    int x1,x2;
    x1 = XCounter_Get_return(xptr);
    XMatrix_mul_Write_matrix_in_V_Words(ptr, 0, (int*)J_fp,factor*N*sizeof(short));
    x2 = XCounter_Get_return(xptr);
    printf("Clock cycles to write into matrix(AXILITE) = %d\n",x2-x1);


    dt_mat writ[N][N];
    XMatrix_mul_Read_matrix_in_V_Words(ptr, 0, (int*)writ, factor*N*sizeof(short));
    for(int i=0;i<N;i++){
        for(int j=0;j<N;j++){
            if(writ[i][j].to_float()!=J[i][j]){
                printf("Matrix element mismatch at %d %d: %f vs %f\n",i,j,writ[i][j].to_float(),J[i][j]);
            }
        }
    }
    ...
    printf("Clock cycles taken to write into partitioned matrix = %d\n",x2-x1);
...
static inline void get_xin()
{
    ...
    rp_AcqGetOldestDataV(RP_CH_1, (uint32_t *)&buff_size, x_in);
...
{
    XMatrix_mul_Write_in_vector_V_Words(ptr, 0, (int *)x_k_fp, 64);
    // int x1, x2;
    // x1= XCounter_Get_return(xptr);
    ...
    XMatrix_mul_Read_out_vector_V_Words(ptr, 0, (int *)fb_fp, 64);
    // x2 = XCounter_Get_return(xptr);
    // printf("Clock cycles taken by FPGA = %d\n",x2-x1);

    // dt_in inv[N];
    // XMatrix_mul_Read_in_vector_V_Words(ptr, 0, (int *)inv, 64);
    ...
    // }
    // x2 = XCounter_Get_return(xptr);
    // printf("Clock cycles taken by CPU = %d\n",x2-x1);

    // for(int i=0;i<N;i++){
    //     printf("i = %d, x_k = %f, x_k_fp = %f, fb = %f, fb_fp = %f\n",i, x_k[i], x_k_fp[i].to_float(), fb[i], fb_fp[i].to_float());
    // }
}
...
void single_iteration(float alpha, float beta, int s, int iteration)
{
    ...
    for (i = 0; i < N_spins; i++)
    {
        n_index = rand() % N_noise;
        // float value = -feedback_terms[i] + noise[n_index];
        float value = -fb_fp[i].to_float() + noise[n_index];
        // printf("feedback_terms[%d] = %f noise = %f value = %f\n",i,feedback_terms[i],noise[n_index],value);
    ...
    // for(int i=0;i<8;i++){
    //     printf("x_k = %f, x_k_fp = %f\n",x_k[i], x_k_fp[i].to_float());
    // }
    ...
    if(traj_write == 1)
    {
        ...
        for (i = 0; i < N_spins; i++)
        {
            fprintf(traj_file, " %f", x_k_fp[i].to_float());
        }
        fprintf(traj_file, "\n");
    }
...
float cut_value()
{
    ...
    for (int i = 0; i < N_spins; i++)
    {
        for (int j = 0; j < N_spins; j++)
        {
            if (x_k_fp[i] * x_k_fp[j] < 0)
            // if (x_k[i] * x_k[j] < 0)
            {
                value = value + OG_J[i][j];
            }
        }
    }
    value /= 2;
